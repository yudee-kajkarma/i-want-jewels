...
export type V2ContentBlock =
  ...
  | { type: "divider" }
  | { type: "grid-layout"; image: string; imagePosition: "left" | "right"; content: V2ContentBlock[] }
...
function InlineFAQItem({ question, answer }: { question: string; answer: string }) {
  ...
  return (
    <div className="overflow-hidden border border-zinc-200 bg-white">
      <button
        ...
        aria-expanded={open}
      >
        <span className="font-poppins text-[16px] font-medium text-[#1f2732]">{question}</span>
        <ChevronDownIcon open={open} />
      </button>
      {open && (
        <div className="border-t border-zinc-100 bg-[#fafaf8] px-5 py-4">
          <p className="font-poppins text-[15px] leading-7 text-[#344256]">{answer}</p>
        </div>
      )}
    </div>
  )
    ...
    <section className="border-t border-zinc-100 px-5 py-14 lg:px-10 lg:py-16">
      <div className="mx-auto max-w-[1400px]">
        ...
        <div className="mx-auto w-[90%] divide-y divide-zinc-100">
          {items.map((item, i) => (
            <InlineFAQItem key={i} question={item.question} answer={item.answer} />
          ))}
        </div>
...
function RenderBlock({ block }: { block: V2ContentBlock }) {
  switch (block.type) {
    case "paragraph":
      return <p className="font-poppins text-[15px] leading-[1.9] text-[#344256]">{block.text}</p>
      ...
      return (
        <ul className="space-y-2 pl-5">
          {block.items.map((item, i) => (
            <li key={i} className="relative font-poppins text-[15px] leading-[1.8] text-[#344256]
    ...
    case "numbered-list":
      return (
        <ol className="list-decimal space-y-2 pl-5">
          {block.items.map((item, i) => (
            <li key={i} className="font-poppins text-[15px] leading-[1.8] text-[#344256] marker:font-semibold marker:text-[#bb923a]">
              {item}
            </li>
          ))}
        </ol>
          ...
          <table className="w-full border-collapse font-poppins text-[14px]">
            ...
            <tbody>
              {block.rows.map((row, r) => (
                <tr key={r} className={r % 2 === 0 ? "bg-white" : "bg-[#fdfaf7]"}>
                  {row.map((cell, c) => (
                    <td
                      key={c}
    ...
    case "see-also":
      return (
        <div className="my-4 inline-block bg-[#fef4f9] px-4 py-2 font-poppins text-[13px]">
          <span className="font-semibold text-[#1f2732]">See also: </span>
          <Link
            to={block.href}
    ...
    case "faq-item":
      return <InlineFAQItem question={block.question} answer={block.answer} />
    ...
    case "grid-layout": {
      ...
      const tableIndex = block.content.findIndex((b) => b.type === "table")
      const hasTable = tableIndex !== -1
      const inColumnBlocks = hasTable ? block.content.slice(0, tableIndex + 1) : block.content
      const belowBlocks = hasTable ? block.content.slice(tableIndex + 1) : []
        ...
        <div className="my-8">
          ...
          <div className="flex flex-col gap-0 md:flex-row md:items-stretch">
            {block.imagePosition === "left" && (
              <div
                className={`relative w-full self-stretch flex-shrink-0 md:w-1/2 overflow-hidden ${!hasTable ? "min-h-[300px]" : ""
                  }`}
                style={{
                  minHeight: 0,
            ...
            <div
              className={`flex w-full flex-col ${hasTable ? "justify-start" : "justify-center"
                } space-y-4 md:w-1/2 md:px-6 py-0`}
            >
              {inColumnBlocks.map((subBlock, sbi) => (
                <RenderBlock key={subBlock.type + sbi} block={subBlock} />
              ))}
            </div>
...
function RenderSection({ section, index }: { section: V2ArticleSection; index: number }) {
  return (
    <div className={index > 0 ? "mt-12 border-t border-zinc-100 pt-10" : ""}>
      {/* Heading */}
      {section.heading && (
        <h2 className={`mb-4 font-play text-[22px] font-semibold leading-snug text-[#1f2732] md:text-[24px] ${index === 0 ? "text-left" : "text-left"}`}>
          {section.heading}
        </h2>
      )}
  ...
  return (
    <article className="group">
      ...
      <div className="pt-4">
        ...
        <p className="mt-1.5 line-clamp-2 text-[13px] leading-6 text-[#344256]">{article.excerpt}</p>
      </div>
    </article>
  )
}
  ...
  return (
    <div className="min-h-screen bg-white font-poppins text-[#1f2732]">
      ...
      <main>
        ...
        <div className="w-full overflow-hidden bg-[#f5ece3]">
          <img
              ...
              <div className="flex w-full flex-col justify-center px-0 md:w-1/2 md:px-10 lg:px-14">
                ...
                <div className="mt-5 space-y-4">
                  {heroIntro.paragraphs.map((para, i) => (
                    <p key={i} className="font-poppins text-[14px] leading-[1.85] text-[#344256]">
                      {para}
                    </p>
                  ))}
                </div>
