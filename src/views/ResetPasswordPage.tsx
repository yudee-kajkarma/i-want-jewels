...
export default function ResetPasswordPage() {
  ...
  async function handleSendOtp() {
    ...
    try {
      const response = await sendOtp({ email: email.trim(), purpose: 'password_reset' })
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(response.message || 'Password reset successful. You can sign in now.')
      navigate('/login', { replace: true })
    } catch {
      setError('Password reset failed. Check email, OTP, and password, then try again.')
    } finally {
  ...
  return (
    <AuthShell
      ...
      asideBody="This flow uses your API endpoints: send OTP with purpose reset, then submit email, otp, and newPassword to reset the account password."
    >
