...
const socialLinks = ['Facebook', 'Instagram', 'YouTube', 'Twitter', 'Pinterest']
...
function getCategoryImage(label: string): string {
  ...
  if (normalizedLabel.includes('necklace') || normalizedLabel.includes('jewellery') || normalizedLabel.includes('jewelry')) {
...
function UtilitySelect({ label }: { label: string }) {
  return (
    <button type="button" className="flex items-center gap-1 text-sm text-zinc-700 transition hover:text-pink-500">
      <span>{label}</span>
      <ChevronIcon />
    </button>
  )
...
export default function Header() {
  ...
  const wishlistCount = useAppSelector((state) => state.wishlist.wishlist?.items.length ?? 0)
  const accountLabel = isAuthenticated ? session?.firstName || session?.username || 'Account' : 'Account'
  const accountName =
  ...
  const visibleShopCategories = useMemo(
    () => (shopCategories.length > 0 ? shopCategories : getShopCategoriesFallback()),
    [shopCategories],
  )
  const shopMenuTiles = useMemo(() => {
    const normalizedCategoryMap = new Map(
      visibleShopCategories.map((category) => [normalizeCategoryLabel(category).toLowerCase(), category]),
    )

    const resolveCategory = (label: string) => normalizedCategoryMap.get(label.toLowerCase()) ?? label
  ...
  function renderDesktopNavLink(link: (typeof activeNavLinks)[number]) {
    if (link.label !== 'Shop' || isAdmin) {
      return <MenuLink key={link.label} label={link.label} to={link.to} className={desktopNavLinkClass} />
    }
  ...
  function handleGlobalSearchSubmit(searchTerm: string, options?: { closeMobileMenu?: boolean }) {
        ...
        <div className="mx-auto hidden max-w-[1480px] items-center justify-between px-4 py-4 text-sm lg:flex lg:px-8">
          <div className="flex items-center gap-8 text-zinc-800">
            {utilityLinks.map((link) => (
              <MenuLink key={link.label} label={link.label} to={link.to} className="transition hover:text-pink-500" />
            ))}
          </div>
      ...
      <header className="sticky top-0 z-30 border-b border-zinc-200 bg-white/95 backdrop-blur">
        <div ref={headerInnerRef} className="mx-auto flex max-w-[1480px] items-center justify-between gap-3 px-4 py-4 md:gap-4 md:py-5 lg:px-8">
          ...
          <div className="flex items-center gap-0.5 sm:gap-2 md:min-w-[235px] md:justify-end lg:min-w-[270px]">
                  ...
                  {isAuthenticated ? (
                    <>
                      <div className="rounded-[18px] bg-[#fff7fb] px-4 py-4">
                        <p className="text-xs font-bold uppercase tracking-[0.12em] text-zinc-400">Signed in</p>
                        <p className="mt-2 text-base font-bold text-[#17110d]">{accountName}</p>
                        <p className="mt-1 text-sm text-zinc-500">{session?.email}</p>
                        <p className="mt-2 text-xs font-bold uppercase tracking-[0.12em] text-[#8b5f43]">{session?.role}</p>
                      </div>
                      {isAdmin ? null : (
                        <>
                          <Link
                    ...
                    <>
                      <div className="rounded-[18px] bg-[#fff7fb] px-4 py-4">
                        <p className="text-xs font-bold uppercase tracking-[0.12em] text-zinc-400">Welcome</p>
                        <p className="mt-2 text-base font-bold text-[#17110d]">Login to your account</p>
                        <p className="mt-1 text-sm text-zinc-500">Sign in or create a new account to save wishlist and cart items.</p>
                      </div>
            ...
            {isAdmin ? (
              <>
                <HeaderAction label="Admin Wishlist" to="/admin/wishlist">
                  <HeartIcon />
                </HeaderAction>
                <HeaderAction label="Admin Cart" to="/admin/cart">
                  <BagIcon />
                </HeaderAction>
              </>
            ) : (
              <>
                <HeaderAction label="Wishlist" count={wishlistCount} to="/wishlist">
                  <HeartIcon />
                </HeaderAction>
                <HeaderAction label="Cart" count={itemCount} to="/cart">
                  <BagIcon />
                </HeaderAction>
              </>
            )}
          ...
          <div className="border-t border-zinc-100 px-4 py-4 lg:hidden">
            <div className="mx-auto max-w-7xl space-y-4">
              <form
                onSubmit={(event) => {
                  event.preventDefault()
                  handleGlobalSearchSubmit(mobileSearchTerm, { closeMobileMenu: true })
                }}
              >
                <label className="flex h-11 items-center gap-3 rounded-xl border border-zinc-200 bg-white px-4 text-sm text-zinc-400 shadow-[0_1px_0_rgba(17,24,39,0.03)]">
                  <SearchIcon />
              ...
              <nav className="grid gap-1 rounded-2xl border border-zinc-200 bg-white p-2">
                {activeNavLinks.map((link) => {
                  if (link.label === 'Shop' && !isAdmin) {
                    return (
                      <div key={link.label} className="rounded-xl border border-[#f1e1d7] bg-[#fffaf7] p-2">
                        <button
                          type="button"
                          aria-expanded={isMobileShopMenuOpen}
                          onClick={() => setIsMobileShopMenuOpen((currentValue) => !currentValue)}
                        ...
                        >
                          <span>{link.label}</span>
                          <span className={`transition-transform duration-300 ${isMobileShopMenuOpen ? 'rotate-180' : ''}`}>
                            <ChevronIcon />
                          </span>
                        </button>

                        <div className={`shop-mobile-menu ${isMobileShopMenuOpen ? 'shop-mobile-menu--open' : ''}`}>
              ...
              <div className="flex items-center justify-between rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm text-zinc-700 sm:hidden">
                <div className="flex items-center gap-4">
                  {isAuthenticated ? (
                    <span className="font-medium text-zinc-900">{accountName}</span>
                  ) : (
                    <>
                      <Link to="/tickets" className="transition hover:text-pink-500" onClick={() => setIsMobileMenuOpen(false)}>
                        <span className="inline-flex items-center gap-1">
                          <Ticket className="h-4 w-4" />
                          Tickets
                        </span>
                      </Link>
                      <Link to="/login" className="transition hover:text-pink-500" onClick={() => setIsMobileMenuOpen(false)}>
                        <span className="inline-flex items-center gap-1">
                          <LogIn className="h-4 w-4" />
                          Login
                        </span>
                      </Link>
                      <Link to="/register" className="transition hover:text-pink-500" onClick={() => setIsMobileMenuOpen(false)}>
                        Register
                      </Link>
                    </>
                  )}
                  {isAdmin ? (
                    <>
                      <Link to="/admin/wishlist" className="transition hover:text-pink-500" onClick={() => setIsMobileMenuOpen(false)}>
                        Wishlists
                      </Link>
                      <Link to="/admin/cart" className="transition hover:text-pink-500" onClick={() => setIsMobileMenuOpen(false)}>
                        Carts
                      </Link>
                    </>
                  ) : (
                    <Link to="/wishlist" className="transition hover:text-pink-500" onClick={() => setIsMobileMenuOpen(false)}>
                      Wishlist
                    </Link>
                  )}
                  {isAuthenticated && !isAdmin ? (
                    <Link to="/profile" className="transition hover:text-pink-500" onClick={() => setIsMobileMenuOpen(false)}>
                      Profile
                    </Link>
                  ) : null}
                  {isAuthenticated && !isAdmin ? (
                    <Link to="/payments/history" className="transition hover:text-pink-500" onClick={() => setIsMobileMenuOpen(false)}>
                      <span className="inline-flex items-center gap-1">
                        <CreditCard className="h-4 w-4" />
                        Payment History
                      </span>
                  ...
                  {isAuthenticated && !isAdmin ? (
                    <Link to="/orders" className="transition hover:text-pink-500" onClick={() => setIsMobileMenuOpen(false)}>
                      <span className="inline-flex items-center gap-1">
                        <PackageSearch className="h-4 w-4" />
                        My Orders
                      </span>
