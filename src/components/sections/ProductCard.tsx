...
export default function ProductCard({
  ...
  const discountPercent = hasDiscount
    ? Math.round(
        ((comparePriceAmount - basePriceAmount) / comparePriceAmount) * 100,
      )
    : 0;
  const productDetailUrl = `/products/${item.slug || item.id}`;
  const isNewArrival =
    item.tags.some((tag) => tag.toLowerCase().includes("new")) || hasDiscount;
  ...
  function commitBuyNow(
    ...
    navigate("/checkout?source=single", { state: { source: "single", draft } });
  }
    ...
    sizePickerOpen && selectedVariant
      ? createPortal(
          <div
            ...
            onClick={(e) => {
              if (e.target === e.currentTarget) setSizePickerOpen(false);
            }}
          >
            <div
            ...
            >
              ...
              <div className="mt-auto flex flex-col gap-2 pt-6">
                <button
                  ...
                  disabled={
                    pickerSizeIdx === null ||
                    (selectedVariant.sizes?.[pickerSizeIdx]?.stock ?? 0) <= 0
                  }
