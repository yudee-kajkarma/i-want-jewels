...
export default function ProductsFilters({
    ...
    const clampedMinimumPrice = Math.min(
        selectedMinimumPrice,
        selectedMaximumPrice,
    );
    const clampedMaximumPrice = Math.max(
        selectedMinimumPrice,
        selectedMaximumPrice,
    );
    const priceRangeSpan = Math.max(maximumPrice - minimumPrice, 1);
    const minimumOffset =
        ((clampedMinimumPrice - minimumPrice) / priceRangeSpan) * 100;
    const maximumOffset =
        ((clampedMaximumPrice - minimumPrice) / priceRangeSpan) * 100;
    ...
    function updatePriceFilter(
        nextMinimumPrice: number,
        nextMaximumPrice: number,
    ) {
        const normalizedMinimumPrice = Math.max(
            minimumPrice,
            Math.min(nextMinimumPrice, nextMaximumPrice),
        );
        const normalizedMaximumPrice = Math.min(
            maximumPrice,
            Math.max(nextMaximumPrice, normalizedMinimumPrice),
        );
            ...
            <section className="border-b border-[#e9e1d8] pb-8">
                ...
                <div className="mt-7 space-y-4">
                    {availableCategories.map((category) => {
                        ...
                        const categoryCount =
                            filterOptions?.categoryCounts?.[
                                category.toLowerCase()
                            ] ??
                            categoryCounts[category.toLowerCase()] ??
                            0;
                        const nextFilters = {
                            ...filters,
                        ...
                        return (
                            ...
                            >
                                <span
                                    className={
                                        isSelected
                                            ? "font-medium text-[#161311] underline underline-offset-4"
                                            : ""
                                    }
                                >
                ...
                <div className="mt-7">
                    <div className="relative h-7">
                        ...
                        <input
                            ...
                            className="iwj-range-dual absolute left-0 top-1/2 h-7 w-full -translate-y-1/2 appearance-none bg-transparent accent-[#1f1f22]"
                        />
                        <input
                            type="range"
                            min={minimumPrice}
            ...
            <section className="border-b border-[#e9e1d8] py-8">
                ...
                <div className="mt-7 flex flex-wrap gap-3">
                    {availableColors.map((colorValue) => {
                        const useMetalFilter =
                            (filterOptions?.metals?.length ?? 0) > 0;
                        const isSelected = useMetalFilter
                            ? filters.metal.some(
                                  (metalValue) =>
                                      metalValue.toLowerCase() ===
                        ...
                        const nextFilters = {
                            ...filters,
                            color: useMetalFilter
                                ? filters.color
                                : isSelected
                                  ? ""
                                  : colorValue,
                            metal: useMetalFilter
                                ? isSelected
                                    ? []
                                    : [colorValue]
                                : filters.metal,
