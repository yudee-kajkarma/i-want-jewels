...
import { translateCategoryLabel } from "../../utils/translateCategoryLabel";
...
export default function HomeBody() {
  ...
  useEffect(() => {
    ...
    async function loadCategories() {
      try {
        ...
        setCategories([]);
        setCollectionGroups([]);
        setCategoriesError(t("home.loadCategoriesError"));
      }
    }
  ...
  useEffect(() => {
    ...
    async function loadProductsByCategory() {
      ...
      try {
        ...
        setFeaturedProducts([]);
        setProductsError(t("home.loadProductsError"));
      } finally {
        if (isMounted) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setCollectionProducts([]);
        setCollectionError(t("home.loadCollectionError"));
      } finally {
        if (isMounted) {
          setIsLoadingCollection(false);
        }
  ...
  const categoryCards: Array<{
    ...
    ...categoryShowcase,
    {
      labelKey: "productDetail.collection",
          ...
          <div className="mt-8 border border-dashed border-zinc-300 bg-white px-6 py-10 text-center">
            <h3 className="text-[14px] font-medium uppercase tracking-[0.18em] text-zinc-800">
              {t("home.noProductsFound", {
                category: translateCategoryLabel(activeCategory, t),
              })}
            </h3>
